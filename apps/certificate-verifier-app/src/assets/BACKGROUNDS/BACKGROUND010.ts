...
<head>
    ...
    <style>
        ...
        .page-background {
            ...
            z-index: -1;
            background: white;
            background-image: url(https://res.cloudinary.com/dkzhem9dj/image/upload/v1773430956/Plantilla-Certificados-MarcaAguaV2_ieklaf.svg);
...
<body style="margin: 0">
    ...
    <div class="bodydiv">
        ...
        <div class="col1">
            <div class="logos-section">
                <img src="https://res.cloudinary.com/dkzhem9dj/image/upload/v1773323598/UniversitarioRU2026_tbflrl.svg" alt="Logo del Universitario Rumiñahui" class="logo1"/>
            ...
            <div class="name-participant-section">
                <h1 class="montserrat-bold-text">El Departamento de Investigación del Instituto Superior Tecnológico Rumiñahui</h1>
                <h1 class="montserrat-bold-text-2">CERTIFICA AL:</h1>
                ...
                <div class="signs-section">
                    <div style="position: relative; display: inline-block;">
                        ...
                        <p style="margin-top: 0cm;font-size: 0.4cm !important;" class="montserrat-text-sign">Ph.D. Marcelo Zambrano</p>
    ...
    <!-- Script para imprimir página y cursiva al nombre del conversatorio -->
    <script>
        function italicizePhrase(element, phrase) {
            const text = element.textContent || '';
            const index = text.indexOf(phrase);
            if (index === -1) return;

            const hasOpenQuote = index > 0 && text[index - 1] === '"';
            const hasCloseQuote = index + phrase.length < text.length && text[index + phrase.length] === '"';

            const start = hasOpenQuote ? index - 1 : index;
            const end = hasCloseQuote ? index + phrase.length + 1 : index + phrase.length;

            const before = text.slice(0, start);
            const italic = text.slice(start, end);
        ...
        function adjustCertificaLabel() {
            const nameEl = document.querySelector('.participant-name');
            if (!nameEl) return;

            const name = nameEl.textContent.trim();
            if (name === 'Ph.D. Ana Zambrano Vizuete') {
                const certificaEl = document.querySelector('.montserrat-bold-text-2');
                if (certificaEl) {
                    certificaEl.textContent = 'CERTIFICA A LA:';
                }
            }
        }

        window.addEventListener('load', function () {
            const montserratEl = document.querySelector('.montserrat-text');
            if (montserratEl) {
                italicizePhrase(montserratEl, 'Acreditación Internacional de Programas y Carreras');
            }
...
`
export default html_BACKGROUND010